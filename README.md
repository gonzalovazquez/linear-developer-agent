...
```
  ...
  │                    GitHub Repository                         │
  │                                                              │
  │  User comments: "@claude add error handling"                │
  └────────────────────────┬────────────────────────────────────┘
                           │
  ...
  │                                                              │
  │  Checks webhook configuration for this repo                  │
  │  Finds: POST https://your-server.vercel.app/webhook/github  │
  └────────────────────────┬────────────────────────────────────┘
                           │
                           │ HTTP POST with comment data
                           │
                           ▼
  ┌─────────────────────────────────────────────────────────────┐
  │            Your Express Server (Vercel)                      │
  │                                                              │
  │  POST /webhook/github receives:                             │
  │  - Comment body: "@claude add error handling"               │
  │  - PR number: 42                                             │
  ...
  │                                                              │
  │  1. Reads PR files from GitHub API ──────────┐              │
  │  2. Sends context to Claude API              │              │
  │  3. Gets updated code from Claude            │              │
  │  4. Commits to GitHub via API ───────────────┤              │
  │  5. Posts reply comment via API ─────────────┤              │
  └──────────────────────────────────────────────┼──────────────┘
                                                 │
                           ┌─────────────────────┴─────────────┐
                           │                                   │
                ...
                │                  │              │                  │
                │ - List files     │              │ - Generate code  │
                │ - Read content   │              │ - Analyze issue  │
                │ - Create commits │              │                  │
                │ - Post comments  │              │                  │
                └──────────────────┘              └──────────────────┘
